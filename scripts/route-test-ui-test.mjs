...
assert.match(html, /id="codexOAuthBtn"/)
assert.match(html, /id="codexOAuthDialog"/)
assert.match(html, /id="credentialEditorTitle"/)
assert.match(html, /id="startCodexOAuthBtn"/)
assert.match(html, /id="completeCodexOAuthBtn"/)
assert.match(html, /id="importCodexOAuthBtn"/)
...
assert.match(html, /data-tab="radar"/)
assert.match(html, /id="radarPanel"/)
assert.match(html, /id="radarFrame"/)
assert.match(html, /data-src="https:\/\/codex-reset-radar\.pages\.dev\/"/)
...
assert.match(main, /function startCodexOAuth/)
assert.match(main, /function pollCodexOAuth/)
assert.match(main, /\/api\/oauth\/codex\/start/)
assert.match(main, /\/api\/oauth\/codex\/complete/)
assert.match(main, /\/api\/oauth\/codex\/import/)
assert.match(main, /provider\.providerType === 'codex_oauth'/)
assert.match(main, /oauthProvider \? 'OAuth 账号' : 'Key 分组'/)
assert.match(main, /addCredentialBtn'\)\.hidden = oauthProvider/)
...
assert.match(main, /function loadRadarFrame/)
assert.match(main, /function scheduleRadarRelease/)
assert.match(main, /RADAR_RELEASE_DELAY_MS = 60000/)
...
assert.match(styles, /\.oauth-dialog/)
assert.match(styles, /\.oauth-status\.success/)
assert.match(styles, /html\[data-theme="dark"\] \.oauth-status/)
...
assert.match(styles, /\.radar-viewport/)
assert.match(html, /class="radar-attribution"/)
assert.match(html, /https:\/\/deng\.codexradar\.com\//)
assert.match(html, /github\.com\/zhigong0506\/local-model-relay\/issues/)
assert.match(styles, /\.radar-attribution/)
