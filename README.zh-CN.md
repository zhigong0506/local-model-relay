...
管理界面的“雷达”入口通过 iframe 按需展示
[Codex Reset Radar](https://codex-reset-radar.pages.dev/)。外部内容、数据和品牌归原站点所有；
本项目不抓取、不修改、不重新分发其数据，仅用于非营利的本地快捷查看，也不代表双方存在
官方合作关系。

推荐访问站主的另一个项目：[登舱](https://deng.codexradar.com/)。如站点所有者希望移除
内嵌入口，请通过本仓库的 [GitHub Issues](https://github.com/zhigong0506/local-model-relay/issues)
联系，维护者会及时处理。
